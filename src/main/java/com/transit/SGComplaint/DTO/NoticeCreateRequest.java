package com.transit.SGComplaint.DTO;

import jakarta.validation.constraints.NotBlank;
import jakarta.validation.constraints.Pattern;
import jakarta.validation.constraints.Size;
import org.springframework.web.multipart.MultipartFile;

import java.util.ArrayList;
import java.util.List;

public class NoticeCreateRequest {

    @NotBlank(message = "공지 분류를 선택해 주세요.")
    @Pattern(regexp = "GENERAL|SYSTEM|SERVICE", message = "올바른 공지 분류를 선택해 주세요.")
    private String category;

    private boolean pinned;

    private boolean popup;

    @NotBlank(message = "공지사항 제목을 입력해 주세요.")
    @Size(max = 100, message = "제목은 100자 이내로 입력해 주세요.")
    private String title;

    @NotBlank(message = "공지사항 내용을 입력해 주세요.")
    @Size(max = 20000, message = "서식을 포함한 공지사항 내용이 너무 깁니다.")
    private String content;

    private List<MultipartFile> contentImages = new ArrayList<>();

    public String getCategory() { return category; }
    public void setCategory(String category) { this.category = category; }
    public boolean isPinned() { return pinned; }
    public void setPinned(boolean pinned) { this.pinned = pinned; }
    public boolean isPopup() { return popup; }
    public void setPopup(boolean popup) { this.popup = popup; }
    public String getTitle() { return title; }
    public void setTitle(String title) { this.title = title; }
    public String getContent() { return content; }
    public void setContent(String content) { this.content = content; }
    public List<MultipartFile> getContentImages() { return contentImages; }
    public void setContentImages(List<MultipartFile> contentImages) {
        this.contentImages = contentImages == null ? new ArrayList<>() : contentImages;
    }
}
